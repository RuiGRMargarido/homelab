...
Proven on LXC 101, Caddy, on 17/09/2026, then on LXC 107, the stopped \*arr stack, and LXC 108, the running monitor, on 18/09/2026: the least valuable guests first, chosen on purpose, with VM 102 last. An import sends nothing to Proxmox. It records in the state that a resource block is an object that already exists, and the test afterwards is the one used everywhere else: `plan` must report no changes.
