...
- Current RAM: **24GB since 08/09/2026** (one 16GB module beside one of the original 8GB; the ceiling is 32GB, confirmed by SMBIOS). **Decided 24/08/2026: no hardware is being bought for now**, so the 32GB upgrade and the storage expansion become conditional options with triggers rather than planned work - see `CHECKLIST.md` Phase 0 and Phase 3. **Also decided 24/08/2026: VM 100 (`mnt-mate`, 8GB) is leaving this hypervisor**, which removes the only real competitor for memory. It is already powered off, so this frees nothing today; what it removes is the standing caveat that shadowed every decision, and 8GB of allocation reserved for something that might restart.
...
- Development environment: **moves to a dedicated physical machine** (decided 24/08/2026). VM 100 `mnt-mate` (Linux Mint) leaves this hypervisor; it is already powered off and will be removed, freeing the 8GB allocation it held. The new machine connects to the **TL-SG608E switch over Ethernet**. Design deferred by choice - this is a forward reference, not planned work yet.
...
- **v2 (Dell OptiPlex 3060 Micro, in progress)**: the new homelab, replacing v1. Current state: Proxmox VE, TrueNAS (VM 102), WireGuard (LXC 103), Caddy (LXC 101), Nextcloud (LXC 104), Jellyfin (LXC 105), the OPNsense firewall VM (106) and the media automation stack (LXC 107), all **created and running**. The development VM `mnt-mate` (100) also lives here but is powered off and **leaving the hypervisor** (decided 24/08/2026). The v1 RAG stack (Open WebUI + Ollama + Qdrant) **has not been recreated** - whether it returns is still open, see Open items.
...
- **RAM impact**: one more consumer added to an already tight budget (see Risks and mitigations). **Ceiling confirmed 03/09/2026: 32GB**, both slots occupied by 8GB modules, so the upgrade is a replacement rather than an addition.
...
- Insufficient RAM: **no longer theoretical** (confirmed 02/08/2026) - the sum of what is *configured* across all VMs and LXCs exceeds the 16GB physically present. It only holds because *actual* usage stays below that, but there is no headroom for k3s or Prometheus/Grafana, and the development VM will grow with Docker and microservices. **Reinforced 12/08/2026** with the media automation stack, which added six containers and pushed the host into swap. **Revised 24/08/2026**: the RAM upgrade is not scheduled either, so it stops being treated as a prerequisite and becomes a trigger-based option. What makes that workable is that the 16GB is not actually short today - TrueNAS holds 8GB, OPNsense 3GB, and the containers fit in the rest. **And VM 100 is leaving the hypervisor** (decided 24/08/2026), which removes the only allocation that could genuinely compete. The consequence is that **Phase 4 splits**, but not where it was first drawn: OpenTofu and Ansible run on the PC and cost the host nothing, and **k3s itself is light** - a single-node control plane fits in 2 to 3GB. What actually needs the RAM is migrating Jellyfin and Nextcloud into it, and the `kube-prometheus-stack`, which alone wants 2 to 4GB.
...
- **The storage cannot sustain the write load asked of it** (added 24/08/2026, after a six-day incident). A single mechanical disk, in a USB dock, with no redundancy and no separate log device, serving NFS to five clients. It reads at 120 MB/s and writes 3GB locally at 91 MB/s, so it is not broken - but under NFS's synchronous commits it stalls, and when it stalls the failure does not degrade, it cascades: ZFS's `txg_sync` blocks, the `nfsd` threads block behind it, every client piles up in uninterruptible sleep, and the host's load average reaches the hundreds. Mitigations applied (see `CHECKLIST.md` Phase 2c) buy headroom without changing that fact. **Revised 24/08/2026**: no hardware is being bought for now, so the correct engineering answer (a mirror, SATA instead of the USB dock, an SSD as a ZFS intent log) becomes a documented option with triggers rather than planned work - see Phase 3. The active response is Phase 2d, which manages the load instead of growing the hardware, and **accepts the mitigations as permanent rather than temporary**: up to five seconds of media writes lost on a crash, and a stalled TrueNAS that waits for a human instead of resetting itself. Both were chosen deliberately, the second because a reset mid-write truncates files silently, which is worse than a stall. **Revised 01/09/2026, and in the mitigations' favour**: the guest's kernel log shows the cascade signature (RCU stalls every 73 seconds for twenty minutes, with `txg_sync` and all four `nfsd` threads blocked) **stopped dead** after the 24/08 changes. The mitigations did not merely buy headroom, they closed that failure class. The underlying fragility is unchanged and the trigger conditions in Phase 3 still stand, but this risk should no longer be read as actively firing.
...
- **The guest can restart itself and the hypervisor will not notice** (added 01/09/2026). On 29 and 30 August TrueNAS rebooted itself with no task in the Proxmox log and no interruption to the QEMU process, which means every host-side indicator, including Proxmox's own uptime column, kept reporting continuity across a restart that really happened. The mitigation supposedly covering this, `-global ICH9-LPC.noreboot=true`, is present and valid on this `q35` machine and did not hold, so the guest reboots along a path it does not gate. **The general lesson outlives the specific bug**: a supervisor's view of a component is a model of that component, not the component, and where the two can diverge the check has to read the thing itself. The restart monitor exists precisely because it asks the guest for its own uptime rather than asking the hypervisor about the process. The cause of the restarts is unresolved and tracked in `CHECKLIST.md` Phase 2c; what is resolved is that they are **not** storage failures, since no ZFS or NFS task was blocked and the vCPU was halted waiting for a timer interrupt the hypervisor never delivered.

## Open items
The full checklist with status (done/pending) for every task is in `docs/CHECKLIST.md`. Decisions still open (not tasks): which app goes into the DMZ zone (publicly exposed via Caddy) - Nextcloud and Jellyfin are already decided as Trusted-only, so the original goal of "exposing 1-2 apps" has no app attached until this is settled; the network zone for the development/LLM-agents VM (Trusted vs. an isolated zone of its own); whether to recreate the v1 RAG stack; and the fate of the old PC (v1) once v2 is operational.
...
- 24/08/2026: **decided not to buy hardware for now**, and the plan revised to stop assuming otherwise. Phase 0's RAM and SSD items and the whole of Phase 3 become **conditional options with triggers**, not scheduled work, and nothing else in the documents may treat them as prerequisites. Three consequences follow, all of them stated rather than left implicit. **The mitigations become permanent**: `sync=disabled` on the media dataset and the disabled watchdog were written as "until Phase 3", and without Phase 3 they are simply the operating configuration, with their costs accepted. **Phase 4 splits**: OpenTofu and Ansible run from the PC and cost the host nothing, so infrastructure-as-code proceeds; only k3s waits. **And monitoring moves up rather than down**: it had been planned as a k3s workload, which put it behind a RAM upgrade that is no longer coming, so it is re-targeted at a small LXC and becomes the highest-value open item in the project. Running permanent workarounds on storage known to be fragile, with no alerting, is now the largest remaining risk - and the failure of 18-24/08 ran for six days unnoticed for exactly that reason.
- 01/09/2026: **a documented mitigation found not to work, and a risk added because of how it failed rather than that it failed**. The claim that a stalled TrueNAS "waits for a human instead of resetting itself" had stood since 24/08 and was false: it reset itself twice. What makes it worth a risk entry rather than a bug fix is the shape of the blind spot. The QEMU process never died, so every host-side signal, Proxmox's uptime column included, reported an unbroken week across two restarts. **Where a supervisor's model of a component can diverge from the component, the check must read the component.** Also revised, in the other direction: the 24/08 storage mitigations are now shown by the kernel log to have closed that failure class outright, not just to have bought headroom.
- 03/09/2026: **32GB of RAM chosen over a second machine, and the reasoning is the deliverable**. Measurements settled it: CPU pressure has a cumulative `full` of zero over nine days, so memory is the only real constraint; and a second OptiPlex bought as a development machine would free nothing here, because the VM it replaces is already powered off. Full study in `CHECKLIST.md`. Two related facts closed the same day: the RAM ceiling is **32GB** with both slots occupied by 8GB modules, answering a question open since February, and `vm.swappiness` was lowered from 60 to 10 because on a hypervisor nearly all anonymous memory is guest RAM, and a guest has no way to see or compensate for its own pages being paged out to disk.
- 03/09/2026: **16GB module bought for 24GB, and k3s decided onto a VM rather than a container**. The memory question resolved into a budget rather than an argument: at 24GB, with TrueNAS at 8GB, OPNsense at 3GB, the containers at their real 933MB and a 4GB k3s VM, about 6GB remains, which is a development VM. The k3s placement had briefly leaned toward an LXC while free memory was 791MB, because a container's limit is a ceiling and a VM's is a reservation. With that constraint lifted the other arguments win: it is an unsupported configuration needing several workarounds, **Kubernetes writes hundreds of iptables rules into whatever kernel it runs in** and in a container that is the same kernel carrying the VLAN bridge and the NFS mounts, a VM snapshots and rolls back whole, and the Proxmox provider for OpenTofu treats VMs as first-class. Also measured: TrueNAS's memory is roughly 2.5GB of operating system plus elastic ARC, not one undifferentiated blob, which is why capping `zfs_arc_max` is the right lever if one is ever needed and shrinking the VM is not.
- 08/09/2026: **RAM upgraded to 24GB, and the measurement taken minutes before the shutdown justified it beyond the argument that bought it**. Swap stood at **7.8GB of 8GB used**, with 1.9GB of TrueNAS's memory and 438MB of OPNsense's paged out to the disk that is this platform's weakest component, accumulated over four days of ordinary household load with the media stack stopped throughout. Lowering `swappiness` to 60 → 10 on 02/09 had been the competing explanation; it was real and it was not sufficient, because a scheduler setting cannot create memory that does not exist. After fitting one 16GB module beside one of the original 8GB: available memory 2.5GB → 14GB, swap zero. The risk entry about guest memory living on disk can be considered addressed, though the cause of the vCPU stalls it was hypothesised to explain remains unproven and the trap stays armed.
- 09/09/2026: **memory redistributed between the two VMs, on measurements rather than caution**. OPNsense went from 3072 to 2048MB (it was using 669MB of 3029) and TrueNAS from 8192 to 6144MB, reversing a mitigation from 24/08. The TrueNAS decision rested on one number that had never been looked at: the ARC held **4.48GiB against a ceiling of 6.74GiB**, so it had 2.26GiB of headroom it never reached for, while sustaining a **99.62% hit rate** across 9.5 million reads. High hit rate and unused headroom are not the same fact, and only the second one justifies a cut. Host available memory went from 10GiB to **15GiB** with swap at essentially zero. The reversal is being watched rather than assumed: I/O pressure and NFS response time are both on the Kuma graphs, which is the first time this project has instruments in place to observe a deliberate change instead of only an incident.
