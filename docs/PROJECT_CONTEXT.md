...
- 06/08/2026: **inicio da Fase 2 - a rede segmentada passou a existir de facto**. Porta trunk configurada no switch, bridge VLAN-aware no Proxmox, adaptador USB→RJ45 como perna WAN, e a VM de firewall dedicada criada (OPNsense, VM 106) com uma interface por zona (WAN `192.168.1.95`, DMZ `10.10.10.1`, Trusted `10.10.20.1`, Management `10.10.30.1`). WireGuard migrado para a DMZ (`10.10.10.10`) e o Proxmox host ganhou uma segunda interface na Management (`10.10.30.2`), mantendo a da rede plana. **Descoberta importante sobre o desenho**: a porta trunk do switch transporta tambem a VLAN 1 sem tag, ou seja, o switch e ao mesmo tempo trunk do homelab e switch normal da rede domestica - a rede de casa **nao** passa pela firewall dedicada, so as VLANs 10/20/30 e que passam. Isto nao estava explicito no esquema original e so foi detetado ao montar fisicamente a Fase 2 (ver `docs/ESQUEMA_LOGICO_REDE.md`).
