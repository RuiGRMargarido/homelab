...
- Current RAM: 16GB. **Decided 24/08/2026: no hardware is being bought for now**, so the 32GB upgrade and the storage expansion become conditional options with triggers rather than planned work - see `CHECKLIST.md` Phase 0 and Phase 3. It fits, provided VM 100 stays powered off while the full service stack runs, which makes it a scheduling constraint rather than a purchase.
