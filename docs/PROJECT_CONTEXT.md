...
- Current RAM: 16GB. **Decided 24/08/2026: no hardware is being bought for now**, so the 32GB upgrade and the storage expansion become conditional options with triggers rather than planned work - see `CHECKLIST.md` Phase 0 and Phase 3. **Also decided 24/08/2026: VM 100 (`mnt-mate`, 8GB) is leaving this hypervisor**, which removes the only real competitor for memory. It is already powered off, so this frees nothing today; what it removes is the standing caveat that shadowed every decision, and 8GB of allocation reserved for something that might restart.
