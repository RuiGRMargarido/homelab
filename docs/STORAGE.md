...
| mnt-mate (VM 110): the development desktop, its editors and whatever is being worked on | the VM's disk, 48GB on Proxmox's local SSD | **No**, and it is a decision rather than an omission: the machine is rebuilt from code, and anything that matters on it belongs in a git repository, which is what it is for. Made room for itself: deleting VM 100 returned 64GB |
| Linux Mint ISO used to create that machine | `local:iso`, on Proxmox's local SSD, 3GB | **No**, and it needs none: pinned to a release and its SHA256. It is kept rather than deleted because it is the only way back to the same starting point, since Mint publishes no cloud image |
...
- 22/09/2026: added the development machine's disk and the Mint ISO to "Where each thing actually lives". Both are deliberately unbacked, and for different reasons worth separating: the machine is rebuilt from code and its contents belong in git, while the ISO is pinned by checksum and can be downloaded again. It also freed space rather than taking it: the VM it replaced returned 64GB and the new one takes 48.
