...
**Refined on 16/09/2026, by the first real VM: unless the privilege named is one of those refused on purpose above.** The first apply of the k3s node downloaded its image and then failed with `403 (/, Sys.Modify)`. What triggered it was a `startup` block setting the boot order: Proxmox treats a guest's boot order as behaviour of the host, not of the guest, and requires `Sys.Modify` on `/` to set it. Following the rule literally would have granted precisely the privilege refused because it can rewrite the host network. The fix was to change the code, removing the block, and nothing was lost, since guests without an order always start after those that have one. The consequence reaches beyond this node: **the boot order of the existing guests stays outside what this token can manage**, alongside VM 102's `args`.
...
One limit to expect when Phase 4 reaches the item about codifying what already exists: VM 102's `args` is a direct passthrough of arguments to KVM, and Proxmox reserves parameters of that kind to `root@pam`. If that holds, `args` cannot be managed by this token and stays a documented manual step, which is a better outcome than meeting it as a permission error halfway through an `apply`. The import met a harder limit first, on 18/09/2026: the pinned provider cannot read VM 102 at all, over the colon at the end of its pool disk's USB path, so VM 102 stays outside OpenTofu until a release fixes that; see `CHECKLIST.md`.
