...
| **Prometheus + Grafana** | k3s, VM 109, namespaces `monitoring` and `monitoring-node` | History and graphs of the cluster and its node since 18/09/2026, and of the Proxmox host, guest by guest, since 21/09/2026. **No alerting**: see `infra/kubernetes/monitoring/README.md` |
...
**The development machine is deliberately not watched, decided 22/09/2026.** VM 110 `mnt-mate` is off whenever nobody is using it, which is most of the time and by design: that is what keeps its 6GB out of the arithmetic the migrations depend on. A Kuma monitor pointed at it would therefore be red almost always, and an alarm that is normally red teaches whoever reads it to ignore the colour, which is the one failure mode this whole document exists to avoid. It is not invisible either: `pve-exporter` reports it like every other guest, because it reads the Proxmox API rather than a list somebody maintains, so the history is in Grafana without anybody adding it anywhere.
...
| `OPNsense firewall` | The firewall's Management interface answers HTTPS 200, with the round trip as response time |
| `Backup config OPNsense` | The nightly config export reached its final line, which `set -e` makes an exact success condition |
...
### Watching the firewall, and why it runs second

Added 04/09/2026, after an evening in which OPNsense spent an hour in a degraded state and **nothing said a word about it**. The failure was visible, since `NFS mounts` and `TrueNAS UI` both traverse the firewall and both went red, but the cause was not: two symptoms elsewhere, and the diagnosis left to a human at midnight.

**A pull monitor is impossible here, and that is the segmentation working.** Uptime Kuma sits on the flat network at `192.168.1.91`, which has no route into `10.10.x.x`, and the firewall's WAN address blocks both ICMP and the web interface by default. Measured from the container: nothing answers. So the check runs from the Proxmox host, which holds `10.10.30.2` on the Management VLAN and is directly adjacent to the firewall's `10.10.30.1`.

**It is the second block in the script, ahead of the NFS check, and that placement is the design.** A dead firewall is a likely *cause* of a hanging NFS check, since all storage traffic has crossed it since the TrueNAS migration of 11/08. Leaving the firewall test behind sixty seconds of NFS timeout would put the check that names the culprit in a queue behind the symptom it explains. That is the same mistake corrected on 01/09 when host health was moved ahead of NFS, made once and worth not making twice.

The message carries the firewall VM's CPU as sampled by Proxmox, because that number has mattered before: on 24/08 it was measured at **166%** during two Jellyfin sessions, throttling storage throughput for everything behind it.

**What is still not covered**, and needs the API key that also unlocks the config backup: the firewall's internal RAM, its disk usage, and the `pf` state table. The state table is the one worth having, because it is the resource specific to being a firewall, and when it fills the symptom is a machine that keeps existing connections while refusing new ones, which reads as an intermittent network fault rather than a resource limit.
...
- **History and graphs beyond the cluster and the host** - Prometheus and Grafana run on k3s since 18/09/2026 and read the Proxmox host since 21/09/2026, through an exporter of its own bound to the Management address. The figures per guest arrived the same day, from the Proxmox API through an exporter of its own with a read-only identity. What they still do not see is TrueNAS from the inside: its pool, its scrubs and its ARC are reported by TrueNAS, not by the hypervisor. Prometheus does not alert, by design; the alerting stays here
...
- **The k3s node (VM 109)**, running since 16/09/2026 with nothing watching it, neither the VM nor its API. Prometheus records the node's metrics since 18/09/2026, but it runs on that same node, so it cannot be the one to report the node gone. A pull monitor is ruled out for the same reason as the firewall's: Uptime Kuma on the flat network has no route into Trusted. The host has one, through rule 3, and both patterns already in the script fit: the guest agent is now enabled on the VM, as it is on TrueNAS, and an anonymous request to `:6443` answering `401` already proves the API is serving. Tracked in `CHECKLIST.md` Phase 5
...
- 21/09/2026: **the hypervisor entered Prometheus, twice over.** Debian's node exporter on the host, bound to its Management address alone, scraped from the cluster under the same job name as the node's exporter, so the chart's dashboards list the two side by side. No firewall rule was needed, in either firewall. Beside it, `prometheus-pve-exporter` in the cluster, reading the Proxmox API with a read-only identity, which answers the question the kernel cannot: how much memory each guest is using.
- 18/09/2026: **Prometheus and Grafana arrived, and the alerting did not move.** `kube-prometheus-stack` runs on k3s with Alertmanager off: it keeps history for the cluster and its node, while every alert still goes through Uptime Kuma, which depends on less than what it watches. Added to the table of what is installed, and the gap it does not close written down: the host and the other guests are not scraped yet, and a Prometheus living on the k3s node cannot report that node's loss.
- 22/09/2026: **a new guest, and the decision not to watch it.** VM 110, the development machine, is off unless somebody is using it, so a Kuma monitor on it would be red most of the time and would train whoever sees it to ignore a red light. It stays out of the alerting on purpose, and stays in the history for free: `pve-exporter` reads the Proxmox API, so it appears in Grafana without anyone adding it to a list.
