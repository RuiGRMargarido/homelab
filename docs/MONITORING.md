...
| **Prometheus + Grafana** | k3s, VM 109, namespaces `monitoring` and `monitoring-node` | History and graphs of the cluster and its node, since 18/09/2026. **No alerting**: see `infra/kubernetes/monitoring/README.md` |
...
- **History and graphs beyond the k3s node** - Prometheus and Grafana run on k3s since 18/09/2026, and see only the cluster and the node it runs on. The Proxmox host, TrueNAS and the other guests are not scraped yet: reaching the host from Trusted needs an exporter there and a firewall rule into Management, a step of its own. Prometheus does not alert, by design; the alerting stays here
