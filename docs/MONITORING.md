...
| **Prometheus + Grafana** | k3s, VM 109, namespaces `monitoring` and `monitoring-node` | History and graphs of the cluster and its node since 18/09/2026, and of the Proxmox host since 21/09/2026. **No alerting**: see `infra/kubernetes/monitoring/README.md` |
...
- **History and graphs beyond the cluster and the host** - Prometheus and Grafana run on k3s since 18/09/2026 and read the Proxmox host since 21/09/2026, through an exporter of its own bound to the Management address. What they still do not see: the figures per guest, which come from the Proxmox API rather than the host's kernel, and TrueNAS. Prometheus does not alert, by design; the alerting stays here
...
- 21/09/2026: **the hypervisor entered Prometheus.** Debian's node exporter on the host, bound to its Management address alone, scraped from the cluster under the same job name as the node's exporter, so the chart's dashboards list the two side by side. No firewall rule was needed, in either firewall.
